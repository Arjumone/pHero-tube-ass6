...
const response = await fetch(` https://openapi.programming-hero.com/api/videos/category/${id}`)
    ...
    idData.forEach((singleIdData)=>{
        ...
        <div class="card bg-base-100 shadow-xl ">
                    <figure class=" relative rounded-lg"><img class="rounded-lg w-[400px] h-[250px] p-3 " src="${singleIdData.thumbnail}" />
                    <p class="absolute mt-48 ms-32 text-white">${singleIdData.others.posted_date}</p>
                    </figure>
                    <h1 class="text-xl font-semibold text-center">${singleIdData.title}</h1>
                    ...
                    <img class="rounded-full h-10 w-10" src="${singleIdData.authors[0].profile_picture}"/>
                        <div class="p-3 text-lg font-medium">
                        ${singleIdData.authors[0].profile_name}
                        </div>
                        <div>
                        <h2 class=""> ${singleIdData.authors[0].verified?`<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                    ...
                    <h2 class=" mb-6 text-center">${singleIdData.others.views} views</h2>
                  </div>
        `
    ...
    //   console.log('hello');
      idData.sort((a, b) => a.others.views - b.others.views);
      handleLoadCard(id);
    });
